        ...
        <div className="flex min-h-screen bg-gray-100">
            ...
            <main className="flex-1 p-10 overflow-y-auto pb-32">
                ...
                <section className="max-w-3xl mx-auto mb-10">
                    <div className="flex flex-col items-center gap-6">
                        ...
                        <div className="text-center">
                            ...
                            <button onClick={handleWithdraw} className="bg-red-500 text-white px-4 py-1 rounded-md text-sm mt-2">회원 탈퇴</button>
                ...
                <section className="max-w-3xl mx-auto bg-white p-8 rounded-lg shadow-md mb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="password" className="w-full border p-2 rounded-md" value={password} onChange={(e) => setPassword(e.target.value)} readOnly={!isEditing} />
                            {errors.password && <p className="text-red-500 text-sm mt-1">{errors.password}</p>}
                        </div>
                        <div>
                            <label className="block font-medium mb-1">비밀번호 확인</label>
                            <input type="password" className="w-full border p-2 rounded-md" value={passwordConfirm} onChange={(e) => setPasswordConfirm(e.target.value)} readOnly={!isEditing} />
                            {passwordMatchMessage && (
                                <p className={`text-sm mt-1 ${passwordMatchMessage === '비밀번호가 일치합니다.' ? 'text-green-500' : 'text-red-500'}`}>{passwordMatchMessage}</p>
                            )}
                        </div>
                        <div>
                        ...
                        <div className="md:col-span-2">
                            ...
                            <input type="text" className="w-full border p-2 rounded-md mb-2 cursor-pointer" value={address} placeholder="주소 검색 클릭" onClick={isEditing ? handleAddressSearch : undefined} readOnly />
                            {errors.address && <p className="text-red-500 text-sm mt-1">{errors.address}</p>}
                            <input type="text" className="w-full border p-2 rounded-md" value={detailAddress} onChange={(e) => setDetailAddress(e.target.value)} placeholder="상세주소 입력" readOnly={!isEditing} />
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <select value={phonePrefix} onChange={(e) => setPhonePrefix(e.target.value)} disabled={!isEditing} className="border p-2 rounded-md">
                                ...
                                <input type="text" className="flex-1 border p-2 rounded-md" value={phoneNumber} onChange={(e) => setPhoneNumber(e.target.value)} placeholder="번호 입력" readOnly={!isEditing} />
                            </div>
                            {errors.phoneNumber && <p className="text-red-500 text-sm mt-1">{errors.phoneNumber}</p>}
                        </div>
                    </div>
                    <div className="flex justify-end mt-6 space-x-3">
                        <button onClick={handleEdit} disabled={isEditing} className="bg-blue-300 text-blue-900 px-4 py-2 rounded-md disabled:opacity-50">정보 편집</button>
                        <button onClick={handleSave} disabled={!isEditing} className="bg-green-300 text-green-900 px-4 py-2 rounded-md disabled:opacity-50">저장</button>
                    </div>
                </section>
                ...
                <section ref={activitySectionRef} className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md">
                    ...
                    <div className={`${isActivityExpanded ? 'max-h-96 overflow-y-scroll' : 'max-h-[140px] overflow-hidden'} transition-all duration-300 space-y-2`}>
