...
import { ChartConfig, HighlightCard, RecentGatherings, RecentPost, RecentReply } from "../type/DashBoardPage.type";
import {
  fetchHighlightCards,
...
const DashBoardPage: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      const [highlightData, gatheringData, postData, commentData] = await Promise.all([
        fetchHighlightCards(),
        fetchRecentGatherings(),
        fetchRecentPosts(),
        fetchRecentReplies(),
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12 relative">
          ...
          <h1 className="text-4xl font-bold text-gray-600 mb-3 pt-14">함께 읽는 즐거움</h1>
            ...
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {gatherings.map((gathering, index) => (
                <div
                  ...
                  <h3 className="font-medium text-gray-900 mb-1">{gathering.title}</h3>
            ...
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {posts.map((post, index) => (
                <div
                  ...
                  <p className="text-gray-600 text-sm mb-2 truncate">{post.content}</p>
                  <div className="flex justify-between text-sm text-gray-500">
                    <span>{post.author}</span>
                    <div className="flex space-x-3">
                      <span>
                        <i className="fas fa-comment text-gray-400 mr-1"></i> {post.comments}
                      </span>
                      <span>
                        <i className="fas fa-heart text-gray-400 mr-1"></i> {post.likes}
                      </span>
                    </div>
                  </div>
                </div>
