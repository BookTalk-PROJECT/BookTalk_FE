...
export const fetchGatheringBoardDetail = async (postId: string): Promise<GetBoardDetailRequest> => {
    ...
    const response = await axios.get<GetBoardDetailRequest>(`${baseURL}/gatheringlist/gatheringboard/${postId}/getBoardDetail`);
...
// 좋아요 토글 (게시글) 
export const toggleLikePost = async (postId: string) => { // <-- 아이디도 추가해줘야함 나중에 로그인 구현 시 추가할 예정
