...
import { MyPageGatheringBoardType } from "../type/MyPageBoardTable";

const MyPageGatheringBoard: React.FC = () => {
  const row: { label: string; key: keyof MyPageGatheringBoardType }[] = [
    { label: "게시물 번호", key: "id" },
  ...
  const initialFilter: { label: string; key: string }[] = [row[2]];

  const postKeys = gatheringBoardPostMockData.length > 0 ? Object.keys(gatheringBoardPostMockData[0]) : [];
